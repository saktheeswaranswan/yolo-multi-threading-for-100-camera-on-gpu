...
# Prompt user for class to scrape
class_to_scrape = input("Enter the class to scrape from the video stream: ")
...
while True:
    ...
    if ret:
        ...
        # Check if the detected class matches the one to scrape and the time is correct
        for i in indices:
            i = i[0]
            if classes[class_ids[i]] == class_to_scrape and current_time in [row[0] for row in times_and_classes if classes[class_ids[i]] in row]:
                # Save cropped image
                x, y, w, h = boxes[i]
                crop_img = frame[y:y+h, x:x+w]
                crop_filename = crop_folder + class_to_scrape + "_" + str(now.timestamp()) + ".jpg"
