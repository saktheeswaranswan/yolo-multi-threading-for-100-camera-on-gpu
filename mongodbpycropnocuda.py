...
def detect_objects(image):
    ...
    if not cat_detected:
        for i, (class_name, crop) in enumerate(zip(class_names, cropped_images)):
            if class_name in ['person', 'dog']:
                timestamp = datetime.datetime.now()
                detection = {
                    'class_name': class_name,
                    'timestamp': timestamp,
                    'image_path': f'detections/{timestamp.strftime("%Y-%m-%d_%H-%M-%S")}_{i}.jpg'
                }
                detections.append(detection)
                cv2.imwrite(detection['image_path'], crop)
...
def detect_objects_webcam():
    ...
    while True:
        ...
        with open('detections.csv', 'a', newline='') as csv_file:
